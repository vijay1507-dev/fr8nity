...
@section('title', 'Edit Partner Showcase Item')

@section('content')
<div class="container-fluid">
    <div class="card">
        <div class="card-header d-flex justify-content-between align-items-center">
            <h4 class="card-title mb-0">Edit Partner Showcase Item</h4>
            <a href="{{ route('admin.partner-showcase.index') }}" class="btn btn-secondary">
                <i class="fas fa-arrow-left me-2"></i>Back to List
            </a>
        </div>
        <div class="card-body">
            <form action="{{ route('admin.partner-showcase.update', $partnerShowcase->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                    ...
                    <div class="col-md-8">
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control @error('title') is-invalid @enderror" 
                                   id="title" name="title" value="{{ old('title', $partnerShowcase->title) }}" required>
                            @error('title')
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control @error('description') is-invalid @enderror" 
                                      id="description" name="description" rows="5" required>{{ old('description', $partnerShowcase->description) }}</textarea>
                            @error('description')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            @if($partnerShowcase->gallery && count($partnerShowcase->gallery) > 0)
                                <div class="mb-2">
                                    <small class="d-block text-muted mb-2">Current gallery images:</small>
                                    <div class="row">
                                        @foreach($partnerShowcase->gallery as $galleryImage)
                        ...
                        <div class="mb-3">
                            ...
                            <input type="number" class="form-control @error('order') is-invalid @enderror" 
                                   id="order" name="order" value="{{ old('order', $partnerShowcase->order) }}" min="0">
                        ...
                        <div class="mb-3">
                            <div class="form-check">
                                <input class="form-check-input" type="checkbox" id="status" name="status" value="1" 
                                       {{ old('status', $partnerShowcase->status) ? 'checked' : '' }}>
                        ...
                        <div class="mb-3">
                            ...
                            <p class="form-control-plaintext">{{ $partnerShowcase->creator ? $partnerShowcase->creator->name : 'N/A' }}</p>
                        </div>

                        <div class="mb-3">
                            <label class="form-label">Created At</label>
                            <p class="form-control-plaintext">{{ $partnerShowcase->created_at->format('M d, Y h:i A') }}</p>
                        </div>
                    </div>
                </div>
                ...
                <div class="d-flex justify-content-center">
                    <button type="submit" class="btn btn-primary">
                        <i class="fas fa-save me-2"></i>Update Partner Showcase Item
                    </button>
                </div>
            </form>
        </div>
